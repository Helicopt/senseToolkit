...
class worker(threading.Thread):
    ...
    def run(self):
        ...
        self.holder.done += 1
        self.holder.rec_json[str(ind)] = OrderedDict(
            {'rec': OrderedDict(res), 'params': self.argv[1]})
            ...
            for i in p_p:
                ...
                for j in range(0, cc):
                    ...
                    if not self.bb[_]:
                        ...
                        merge_p = OrderedDict(
                            {nm[k]: real_p[k] for k in range(len_p)})

                        t = worker(task=(self.run_func, self.eva_func),
                                   argv=(self, merge_p, _))
                        t.setDaemon(True)
                        t.start()
                        job_cnt += 1
                ...
                if best is None or self.cmp(mx,  best):
                    ...
                    bid = gen_code(st, p)
                    self.logger.info(
                        'update best: %.3f, task_id %d' % (best, bid))
