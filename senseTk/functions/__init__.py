...
def drawOnImg(im, D, color=(255, 0, 0), bold=0.5, stamp=False, conf=False):
    ...
    offset = 16
    cv2.putText(im, '%d' % D.uid, D.lt(2, offset),
                cv2.FONT_HERSHEY_DUPLEX, bold, color)
    offset += int(32*bold)
    if stamp:
        cv2.putText(im, '%d' % D.fr, D.lt(2, offset),
                    cv2.FONT_HERSHEY_DUPLEX, bold, color)
        offset += int(32*bold)
    if conf:
        cv2.putText(im, '%.2f' % D.conf, D.lt(2, offset),
                    cv2.FONT_HERSHEY_DUPLEX, bold, color)
        offset += int(32*bold)
...
def autoPattern(path, loose_fmt=True):
    ...
    if isinstance(path, str):
        if os.path.isfile(path):
            ...
            fns = list(filter(lambda x: not re.match(
                '^\.', x), os.listdir(path)))
        else:
            fns = [path]
    else:
...
def compare(d1, d2, k1, k2, eps=1e-6, max_mistakes=10, miscnt=0):
    ...
    def assertType(d1, d2, k1, k2):
        if type(d1) != type(d2):
            print('typediff: %s (%s) vs %s (%s)' %
                  (k1, type(d1), k2, type(d2)))
            return False
        return True
    ...
    def assertKey(d1, d2, k1, k2):
        if len(set(d1.keys()) - set(d2.keys())) != 0 or len(set(d2.keys()) - set(d1.keys())) != 0:
            print('keydiff: %s (%s) vs %s (%s)' % (k1, set(d1.keys()) -
                                                   set(d2.keys()), k2, set(d2.keys()) - set(d1.keys())))
            return False
        return True
    ...
    if isinstance(d1, (list, tuple)):
        ...
        for k, (i, j) in enumerate(zip(d1, d2)):
            ...
            miscnt = c
            if not f:
                return False, kwargs['miscnt']
    if isinstance(d1, dict):
        found = True
        if not assertKey(d1, d2, k1, k2):
            return False, miscnt + 1
        for k in d1.keys():
            f, c = compare(d1[k], d2[k], k1+'.%s' %
                           str(k), k2+'.%s' % str(k), **kwargs)
            kwargs['miscnt'] = c
            miscnt = c
            if not f:
                return False, kwargs['miscnt']
    if isinstance(d1, (int, str, bytes)):
        found = True
        if not assertValue(d1, d2, k1, k2):
            return False, miscnt
    if isinstance(d1, float):
        found = True
        if not assertClose(d1, d2, k1, k2):
            return False, miscnt
    if isinstance(d1, set):
        found = True
        if not assertSet(d1, d2, k1, k2):
            return False, miscnt + 1
    ...
    for i, k1 in enumerate(keys):
        ...
        for j, k2 in enumerate(keys):
            if i >= j:
                continue
            print('--'*20)
            f, c = compare(d[k1], d[k2], k1, k2, eps=eps,
                           max_mistakes=max_value_diff)
            print('--'*20)
            if f:
                print('%s is same as %s (eps %a, mistake %d)' %
                      (k1, k2, eps, c))
            else:
                print('%s differs from %s (eps %a, mistake %d)' %
                      (k1, k2, eps, c))
