...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        'python -m senseTk.apps.VI', description='Tool for Video-Imageset exchange.')
    parser.add_argument('source', help='source video/imgset')
    parser.add_argument('destination', help='target video/imgset')
    parser.add_argument('-t', '--type', default='auto', type=str, choices=[
                        'auto', 'img', 'video'], help='target type, auto is to change to different from source')
    parser.add_argument('-i', '--inverse', action='store_true',
                        help='switch src and dest')
    ...
    parser.add_argument('--ofmt', default='', type=str,
                        help='decide output imgset format')
    parser.add_argument('--ostart', default=1, type=int,
                        help='output imgset start offset')
    ...
    if args.ofmt == '':
        ...
        b = VideoClipWriter(dst, typ, size=a.ImgSize(),
                            fmt=args.ofmt, start=args.ostart)
    streamForward(a, b)
    a.release()
    b.release()
